  ...
  (function(webim) {
                  ...
                  for (var j in resp.UserProfileItem[i].ProfileItem) {
                    switch (resp.UserProfileItem[i].ProfileItem[j].Tag) {
                      ...
                      case 'Tag_Profile_IM_Image':
                        var image =
                          resp.UserProfileItem[i].ProfileItem[j].Value;
    ...
    var proto_getMsgs = function(cookie, syncFlag, cbOk, cbErr) {
      ...
      ConnManager.apiCall(
        SRV_NAME.OPEN_IM,
        'getmsg',
        { Cookie: cookie, SyncFlag: syncFlag },
        function(resp) {
          ...
          if (isOn) {
            ...
            if (selSess._impl.type == SESSION_TYPE.C2C) {
              ...
              proto_c2CMsgReaded(
                ...
                function(err) {
                  log.error(
                    '[setAutoRead}: c2CMsgReaded failed:' + err.ErrorInfo
                  );
                }
              );
    ...
    var MsgManager = new function() {
      ...
      var handlerOrdinaryAndTipC2cMsgs = function(eventType, groupMsgArray) {
        ...
        for (var groupId in groupMsgMap) {
          var tempCount =
            groupMsgMap[groupId].max - groupMsgMap[groupId].min + 1; //收到的新的群消息数
        ...
        if (onMsgCallback && new_group_msgs.length)
          onMsgCallback(new_group_msgs);
      };
            ...
            function(resp) {
              for (var i in resp.EventArray) {
                ...
                switch (e.Event) {
                  ...
                  default:
                    log.error(
                      'longpolling收到未知新消息类型: Event=' + e.Event
                    );
                    break;
                }
              }
      ...
      this.getC2CHistoryMsgs = function(options, cbOk, cbErr) {
        ...
        if (options.MaxCnt > 15) {
          if (cbErr) {
            cbErr(
              tool.getReturnError('MaxCnt can not be greater than 15', -15)
            );
            return;
          }
          return;
    ...
    //基本接口
    //登录
    webim.login = webim.init = function(
      loginInfo,
      listeners,
      opts,
      cbOk,
