...
function onMsgNotify(newMsgList, cb) {
  // let newMsg;
  ...
  switch (opType) {
    case webim.GROUP_TIP_TYPE.JOIN: //加入群
      ...
      for (var m in userIdList) {
        text += userIdList[m] + ',';
        if (
          userIdList.length > WEB_IM_GROUP_TIP_MAX_USER_COUNT &&
          m == maxIndex
        ) {
...
function tlsGetUserSig(res) {
  ...
  if (res.ErrorCode == webim.TLS_ERROR_CODE.OK) {
    ...
    loginInfo.sdkAppID = loginInfo.appIDAt3rd = Number(
...
function onApplyJoinGroupAcceptNotify(notify) {
  ...
  var content =
    notify.Operator_Account + '同意你的加群申请，附言：' + notify.RemarkInfo;
  showGroupSystemMsg(
    notify.ReportType,
    reportTypeCh,
    notify.GroupId,
...
function onApplyJoinGroupRefuseNotify(notify) {
  ...
  var content =
    notify.Operator_Account + '拒绝了你的加群申请，附言：' + notify.RemarkInfo;
  showGroupSystemMsg(
    notify.ReportType,
    reportTypeCh,
    notify.GroupId,
...
function showGroupSystemMsg(
  ...
  var sysMsgStr =
    ...
    msg_content +
    ', 时间=' +
    webim.Tool.formatTimeStamp(msg_time);
